  ...
  <div class="mainleft">
   <ul>
	<c:if test="${readerId=='user' }">
   		<li><a href="/BookSystem/bookinfo/chaxunlist.do?role=user">书籍信息 </a></li>
  ...
  <div class="mainright">
   <div class="content">
   	<div style="margin-top:15px; text-align:center;">
   		<c:if test="${readerId=='user' }">
		   	<form  action="/BookSystem/infopublish/list.do?role=user" method="post" id ="queryform">
	   			...
	   			<img style="top: 9px;position: relative;" width="30px" height="30px" src="../images/find.png" id="search"/>
		   		</span>
	   		</form>	
	   </c:if>
	    <c:if test="${readerId=='root' }">
						...
						<c:forEach items="${pageData.list}" var="item" varStatus="vs">
							<tr>
								...
								<td style="text-align:center;">
									<c:if test="${readerId=='user' }">
										<a href="javascript:void(0)" id = "look" name = "look" onclick = "look('${item.id}');"><img width="20px" height="20px" src="../images/detail.png"></img></a>
								    </c:if>
									<c:if test="${readerId=='root' }">
